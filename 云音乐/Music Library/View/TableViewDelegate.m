...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    else if (tableView == self.rightTableView)
    {
        ...
        [self RequestForSongDetails:model.hashId];
...
#pragma mark - 请求歌曲详情
- (void)RequestForSongDetails:(NSString *)hash
{
   [self.musicModelAry removeAllObjects];
   [AFNTool GET:[NSString stringWithFormat:@"http://www.kugou.com/yy/index.php?r=play/getdata&hash=%@",hash] parameters:nil success:^(id result) {
       NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:result options:0 error:nil];
